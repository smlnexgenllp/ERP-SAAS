...
export default function GateEntryCreate() {
  ...
  // ────────────────────────────────────────────────
  //  FIXED: Correctly extract REAL item ID from item_details.id
  // ────────────────────────────────────────────────
  const getItemId = (itemData) => {
    if (!itemData) return null;

    // Most important: use the real Item ID from nested item_details
    const realItemId = Number(itemData?.item_details?.id);
    if (!isNaN(realItemId) && realItemId > 0) {
      return realItemId;
    }

    // Fallbacks for other possible structures (older data?)
    const fallbacks = [
      itemData?.item?.id,
      itemData?.item,
      itemData?.item_id,
    ...
    console.warn("No valid item ID found in:", itemData);
    return null;
  };
  ...
  const handleSubmit = async (po) => {
    ...
    console.log(`PO #${po.id} raw items from API:`, JSON.stringify(po.items, null, 2));

    const itemsPayload = po.items
      .map((poItem, index) => {
        // ── Only trust this path ──
        let itemId = Number(poItem?.item_details?.id);

        // If that fails, try very common fallback names (just in case API structure changed)
        if (!itemId || isNaN(itemId)) {
          itemId = Number(
            poItem?.item?.id ||
            poItem?.item_id ||
            poItem?.id          // sometimes the PO item itself has the master item id
          );
        }

        if (!itemId || isNaN(itemId) || itemId < 1) {
          console.warn(`PO item #${index} has no valid item ID`, poItem);
          return null;
        }
    ...
    const badItems = itemsPayload.filter(i => typeof i.item !== 'number' || !Number.isInteger(i.item));
    if (badItems.length > 0) {
      console.error("BUG: Non-number item IDs about to be sent!", badItems);
      alert("Cannot submit: internal error with item IDs. Check console.");
      return;
    }
    ...
    challan_number: form.challan_number.trim(), // ✅ correct
    items: itemsPayload,
  };

    console.log("Sending:", payload);  // ← keep this
        ...
        if (data.items?.[0]?.item) {
          ...
          msg = "Items validation failed:\n" +
            data.items
              .map((errObj, i) => {
                if (!errObj) return "";
                return `Item ${i + 1}: ${Object.values(errObj).flat().join(" | ")}`;
              })
              .filter(Boolean)
              .join("\n");
        } else if (data.po) {
          msg = `PO error: ${data.po}`;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-cyan-300">Gate Entry</h1>
          </div>
                      ...
                      return (
                        <React.Fragment key={po.id}>
                          <tr
                            ...
                            className={`cursor-pointer transition-colors ${remaining === 0 ? "opacity-60 bg-gray-800/20" : "hover:bg-gray-800/50"
                          ...
                          >
                            ...
                            <td className="px-6 py-5 text-center">
                              {geStatus.created ? (
                                <span className="text-green-400 font-medium">{geStatus.number}</span>
                              ) : (
                                "Pending"
                              )}
                            </td>
                            <td className="px-6 py-5 text-center">
                              <span
                                className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${remaining > 0 ? "bg-yellow-900/50 text-yellow-300" : "bg-green-900/50 text-green-300"
                                  }`}
                              >
                              ...
                              <td colSpan={7} className="p-6">
                                <div className="space-y-8">
                                  ...
                                  {Object.keys(serverErrors).length > 0 && (
                                    <div className="bg-red-900/40 border border-red-700 rounded-lg p-4 text-red-200 text-sm whitespace-pre-wrap font-mono">
                                      ...
                                      <table className="w-full">
                                        ...
                                        <tbody className="divide-y divide-cyan-900/30">
                                          {(po.items || []).map((item, idx) => {
                                            const itemId = getItemId(item);
                                            if (!itemId) {
                                              console.warn("Skipped invalid item row:", item);
                                              return null;
                                            }
                                              ...
                                              <tr key={idx} className="hover:bg-gray-800/40 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                  <input
                                                    ...
                                                    className={`w-full bg-gray-800 border ${isOver ? "border-red-600 focus:border-red-500" : "border-gray-700 focus:border-cyan-600"
                                                      } rounded-lg px-3 py-2 text-center focus:outline-none transition-colors`}
                                                    disabled={submitting}
                                                  />
                                                </td>
                                                <td className="px-6 py-4 text-right">₹ {price.toFixed(2)}</td>
                                                <td className="px-6 py-4 text-right text-cyan-200 font-medium">
                                                  ₹ {(Number(delivered || 0) * price).toFixed(2)}
                                                </td>
                                              </tr>
                                            );
                                          })}
                                  ...
                                  <div className="bg-gray-800/50 border border-cyan-900/30 rounded-lg p-6">
                                    <h3 className="text-lg font-semibold text-cyan-300 mb-5">Gate Entry Details</h3>
                                    ...
                                    <div className="mt-8 flex justify-end">
                                      <button
                                        ...
                                        disabled={submitting}
                                        className={`flex items-center gap-3 px-8 py-3.5 bg-gradient-to-r from-green-700 to-green-600 hover:from-green-600 hover:to-green-500 text-white font-medium rounded-lg transition-all shadow-lg shadow-green-900/40 ${submitting ? "opacity-60 cursor-not-allowed" : ""
                                          }`}
                                      >
                                        <FiSave size={18} />
