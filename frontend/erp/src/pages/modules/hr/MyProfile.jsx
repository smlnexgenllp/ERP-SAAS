...
export default function MyProfile() {
  ...
  const [reimbursementTab, setReimbursementTab] = useState("pending"); // New: pending | completed
  ...
  const fetchTrainingStatus = async () => {
    try {
      ...
      setTrainingCompleted(res.data.completed);
  ...
  const fetchMyReimbursements = async () => {
    ...
    try {
      ...
      // Ensure only current user's reimbursements are shown (API should already filter by logged-in user)
  ...
  const filteredReimbursements = reimbursements.filter((r) => {
    ...
    return r.status && ["approved", "rejected"].includes(r.status.toLowerCase());
  });
    ...
    <div className="min-h-screen bg-gray-900 text-gray-800">
      ...
      {trainingCompleted && (
        <div className="max-w-7xl mx-auto px-8 mt-6">
          <div className="bg-green-50 border border-green-500 text-green-700 px-6 py-4 rounded-xl flex items-center gap-3 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              >
                <span className="flex items-center gap-3">
                  <Calendar className="w-5 h-5 text-blue-600" /> Apply Leave
                </span>
              ...
              >
                <span className="flex items-center gap-3">
                  <Clock className="w-5 h-5 text-purple-600" /> Request Permission
                </span>
                <ChevronRight className="w-5 h-5 text-gray-400" />
              </button>
              <button
                onClick={openVideosModal}
                className="w-full text-left px-4 py-3 bg-indigo-50 hover:bg-indigo-100 rounded-lg flex items-center justify-between transition"
              >
                <span className="flex items-center gap-3">
                  <PlayCircle className="w-5 h-5 text-indigo-600" /> View Training Videos
                </span>
                <ChevronRight className="w-5 h-5 text-gray-400" />
              </button>
              <button
                onClick={() => setShowReimbursementModal(true)}
                className="w-full text-left px-4 py-3 bg-emerald-50 hover:bg-emerald-100 rounded-lg flex items-center justify-between transition"
              >
                <span className="flex items-center gap-3">
                  <Gift className="w-5 h-5 text-emerald-600" /> Request Reimbursement
                </span>
                <ChevronRight className="w-5 h-5 text-gray-400" />
              </button>
              <button
                ...
                className="w-full text-left px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-lg flex items-center justify-between transition"
              >
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-8">
            ...
            <div className="flex border-b border-gray-200 mb-6 text-sm font-medium">
              ...
              <button
                ...
                className={`px-6 py-3 ${activeTab === "reimbursement" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-500 hover:text-gray-700"}`}
              >
                Reimbursements
              </button>
            </div>
                ...
                {uploadedDocs.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {uploadedDocs.map((doc) => (
                      <div
                        key={doc.id}
                        className="flex items-center justify-between bg-gray-50 p-5 rounded-lg hover:bg-gray-100 transition"
                      >
                        <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <button
                  ...
                  disabled={
                    ...
                    Object.keys(newDocs).every((key) =>
                      uploadedDocs.some((doc) => doc.title === key)
                    )
                  }
              ...
              <div>
                ...
                <div className="flex border-b border-gray-200 mb-6 text-sm font-medium">
                  <button
                    ...
                    className={`px-6 py-3 ${reimbursementTab === "pending" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-500 hover:text-gray-700"}`}
                  >
                    Pending
                  </button>
                  <button
                    onClick={() => setReimbursementTab("completed")}
                    className={`px-6 py-3 ${reimbursementTab === "completed" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-500 hover:text-gray-700"}`}
                  >
                    Completed
                  </button>
                </div>
                ...
                {reimbursementsLoading ? (
                  <p className="text-gray-500 italic text-center py-8">Loading...</p>
                  ...
                  {leaveHistory.length > 0 && (
                    <div>
                      ...
                      {leaveHistory.map((l) => (
                        <div
                          key={l.id}
                          className="bg-gray-50 p-4 rounded-lg mb-3 flex justify-between items-center"
                        >
                          <div>
            ...
            <div className="text-center">
              ...
              <div className="grid grid-cols-7 gap-2 mt-6 text-sm">
                {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
                  (day) => (
                    <div key={day} className="font-bold text-gray-600 py-2">
                      {day.charAt(0)}
                    </div>
                  )
                )}
            ...
            <form onSubmit={submitLeave} className="space-y-4">
              ...
              <div className="flex gap-4">
                ...
                <button
                  type="button"
                  onClick={() => setShowLeaveModal(false)}
                  className="flex-1 py-3 bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold rounded-lg transition"
                >
                  Cancel
                </button>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg">
            ...
            <form onSubmit={submitReimbursement} className="space-y-4">
              <input
                ...
                onChange={(e) =>
                  setReimbursement({ ...reimbursement, amount: e.target.value })
                }
                required
              />
              <input
              ...
              <textarea
                ...
                onChange={(e) =>
                  setReimbursement({ ...reimbursement, reason: e.target.value })
                }
                required
              />
              <div className="flex gap-4">
                <button
                  type="submit"
                  className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg transition"
                >
                  Submit Request
                </button>
                <button
            ...
            <div className="p-8">
              {videosLoading ? (
                ...
                <>
                  ...
                  {trainingCompleted ? (
                    <div className="mt-12 text-center text-green-700 font-bold text-xl flex items-center justify-center gap-3">
                    ...
                    allCompleted && (
                      <div className="mt-12 text-center">
                        <button
                          ...
                          className="px-10 py-5 bg-green-600 hover:bg-green-700 text-white font-bold text-xl rounded-lg"
                        >
